...
test_job = Job(
    ...
    action("actions/checkout@v4", name="Checkout code")
).add_step(
    action(
    ...
    run("pytest", name="Run tests")
)

# Build job (depends on test)
build_job = Job(
    runs_on="ubuntu-latest"
).set_needs("test").add_step(
    action("actions/checkout@v4", name="Checkout code")
).add_step(
    action("actions/setup-python@v4", name="Setup Python", python_version="3.11")
).add_step(
    run("pip install build", name="Install build tools")
).add_step(
    run("python -m build", name="Build package")
).add_step(
    action(
