...
from .triggers import on_push, on_pull_request, on_release
from .strategy import strategy, matrix
...
def python_ci_template(
    ...
    workflow = Workflow(
        name=name,
        on=[
            on_push(branches=branches),
            on_pull_request(branches=branches)
        ]
    )
...
def node_ci_template(
    ...
    if package_manager == "npm":
        ...
        test_job.add_step(run("yarn install --frozen-lockfile", name="Install dependencies"))
