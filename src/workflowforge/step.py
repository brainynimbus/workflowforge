...
class Step(BaseModel, ABC):
    ...
    name: Optional[str] = Field(None, description="Nombre del step")
    id: Optional[str] = Field(None, description="ID único del step")
    if_condition: Optional[str] = Field(None, alias="if", description="Condición para ejecutar el step")
    ...
    env: Optional[Dict[str, str]] = Field(None, description="Variables de entorno del step")
...
class ActionStep(Step):
    ...
    with_params: Optional[Dict[str, Any]] = Field(None, description="Parámetros de la action")
...
class RunStep(Step):
    ...
    shell: Optional[str] = Field(None, description="Shell a usar")
    working_directory: Optional[str] = Field(None, alias="working-directory", description="Directorio de trabajo")
...
def action(uses: str, name: Optional[str] = None, with_: Optional[Dict[str, Any]] = None, **kwargs) -> ActionStep:
...
def run(command: str, name: Optional[str] = None, shell: Optional[str] = None) -> RunStep:
    """Crea un RunStep de manera conveniente."""
    return RunStep(run=command, name=name, shell=shell)