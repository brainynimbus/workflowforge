...
class BuildPhase(BaseModel):
    ...
    run_as: Optional[str] = Field(None, description="Linux user to run commands")
    on_failure: Optional[str] = Field(None, description="Action on failure")
    runtime_versions: Optional[Dict[str, str]] = Field(None, description="Runtime versions")
    commands: List[str] = Field(default_factory=list, description="Commands to execute")
    finally_commands: Optional[List[str]] = Field(None, description="Finally commands")
...
class BuildEnvironment(BaseModel):
    ...
    parameter_store: Optional[Dict[str, str]] = Field(None, description="Parameter Store variables")
    secrets_manager: Optional[Dict[str, str]] = Field(None, description="Secrets Manager variables")
    exported_variables: Optional[List[str]] = Field(None, description="Exported variables")
    git_credential_helper: Optional[bool] = Field(None, description="Git credential helper")
...
class BuildArtifacts(BaseModel):
    ...
    discard_paths: Optional[bool] = Field(None, description="Discard paths")
    base_directory: Optional[str] = Field(None, description="Base directory")
    exclude_paths: Optional[List[str]] = Field(None, description="Paths to exclude")
    enable_symlinks: Optional[bool] = Field(None, description="Enable symlinks")
    s3_prefix: Optional[str] = Field(None, description="S3 prefix")
...
class BuildCache(BaseModel):
    ...
    fallback_keys: Optional[List[str]] = Field(None, description="Fallback keys")
    action: Optional[str] = Field(None, description="Cache action")
...
class BuildSpec(BaseModel):
    ...
    run_as: Optional[str] = Field(None, description="Global run-as user")
    env: Optional[BuildEnvironment] = Field(None, description="Environment configuration")
    install: Optional[BuildPhase] = Field(None, description="Install phase")
    pre_build: Optional[BuildPhase] = Field(None, description="Pre-build phase")
    build: Optional[BuildPhase] = Field(None, description="Build phase")
    post_build: Optional[BuildPhase] = Field(None, description="Post-build phase")
    artifacts: Optional[BuildArtifacts] = Field(None, description="Artifacts configuration")
    cache: Optional[BuildCache] = Field(None, description="Cache configuration")
    reports: Optional[Dict[str, Dict[str, Any]]] = Field(None, description="Reports configuration")
    ...
    def to_yaml(self) -> str:
        ...
        if self.env:
            ...
            if self.env.git_credential_helper is not None:
                env_data["git-credential-helper"] = "yes" if self.env.git_credential_helper else "no"
            if env_data:
                data["env"] = env_data
        ...
        if self.artifacts:
            ...
            if self.artifacts.discard_paths is not None:
                artifacts_data["discard-paths"] = "yes" if self.artifacts.discard_paths else "no"
            ...
            if self.artifacts.enable_symlinks is not None:
                artifacts_data["enable-symlinks"] = "yes" if self.artifacts.enable_symlinks else "no"
    ...
    def save(self, filepath: str = "buildspec.yml", generate_readme: bool = False, use_ai: bool = True, generate_diagram: bool = True) -> None:
        ...
        if generate_readme:
            readme_path = filepath.replace('.yml', '_README.md').replace('.yaml', '_README.md')
