...
class Workflow(BaseModel):
    ...
    on: Union[Trigger, List[Trigger], Dict[str, Any]] = Field(..., description="Eventos que disparan el workflow")
    jobs: Dict[str, Job] = Field(default_factory=dict, description="Jobs del workflow")
    env: Optional[Dict[str, str]] = Field(None, description="Variables de entorno globales")
    defaults: Optional[Dict[str, Any]] = Field(None, description="Configuraciones por defecto")
    concurrency: Optional[Union[str, Dict[str, Any]]] = Field(None, description="Control de concurrencia")
    permissions: Optional[Dict[str, str]] = Field(None, description="Permisos del workflow")
    ...
    def to_yaml(self) -> str:
        ...
        workflow_dict = {
            ...
            "jobs": {job_id: job.model_dump(exclude_none=True) for job_id, job in self.jobs.items()}
        }
    ...
    def save(self, filepath: str = None, generate_readme: bool = False, use_ai: bool = True, generate_diagram: bool = True) -> None:
    ...
    def _serialize_triggers(self) -> Union[str, List[str], Dict[str, Any]]:
