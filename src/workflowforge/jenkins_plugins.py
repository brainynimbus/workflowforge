...
class GitCheckout(BaseModel):
    ...
    credentials_id: Optional[str] = Field(None, description="Jenkins credentials ID")
    shallow: Optional[bool] = Field(None, description="Shallow clone")
...
class DockerPlugin(BaseModel):
    ...
    args: Optional[str] = Field(None, description="Docker run arguments")
    registry_url: Optional[str] = Field(None, description="Docker registry URL")
    credentials_id: Optional[str] = Field(None, description="Registry credentials")
...
class SlackNotification(BaseModel):
    ...
    token_credential_id: Optional[str] = Field(None, description="Slack token credential ID")
...
class EmailNotification(BaseModel):
    ...
    attach_log: Optional[bool] = Field(None, description="Attach build log")
    
    def to_step(self) -> str:
        """Convert to email step."""
        step = f"emailext to: '{self.to}', subject: '{self.subject}', body: '{self.body}'"
        if self.attach_log:
...
class ArtifactArchiver(BaseModel):
    ...
    allow_empty: Optional[bool] = Field(None, description="Allow empty archive")
    fingerprint: Optional[bool] = Field(None, description="Fingerprint artifacts")
    only_if_successful: Optional[bool] = Field(None, description="Only if build successful")
...
class JUnitPublisher(BaseModel):
    ...
    allow_empty: Optional[bool] = Field(None, description="Allow empty results")
    keep_long_stdio: Optional[bool] = Field(None, description="Keep long stdio")
...
def git_checkout(url: str, branch: str = "main", credentials_id: Optional[str] = None) -> GitCheckout:
    """Create Git checkout configuration."""
    return GitCheckout(url=url, branch=branch, credentials_id=credentials_id)

def docker_run(image: str, command: str, args: Optional[str] = None) -> str:
    """Create Docker run step."""
    docker = DockerPlugin(image=image, args=args)
    return docker.to_step(command)

def slack_notify(channel: str, message: str, color: Optional[str] = None) -> str:
