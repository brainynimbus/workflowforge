...
GITHUB_ACTIONS_SCHEMA = {
    ...
    "valid_job_fields": [
        "runs-on", "steps", "needs", "if", "strategy", "environment",
        "permissions", "outputs", "env", "defaults", "timeout-minutes"
    ],
    "valid_step_fields": [
        "name", "uses", "run", "with", "env", "if", "id", "continue-on-error"
    ]
}
            ...
            if "steps" in job_config:
                for i, step in enumerate(job_config["steps"]):
                    ...
                    if "uses" not in step and "run" not in step:
                        errors.append(f"Step {i} in job '{job_name}' must have either 'uses' or 'run'")
                    
                    # Validate step fields
                    for field in step.keys():
                        if field not in GITHUB_ACTIONS_SCHEMA["valid_step_fields"]:
                            errors.append(f"Invalid step field in job '{job_name}', step {i}: {field}")
